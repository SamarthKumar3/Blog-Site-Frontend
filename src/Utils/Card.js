...
import { normalizeImageUpload, capitalize } from '@/Utils/Misc';
...
const Card = ({ article }) => {
    return (
        <div className='flex flex-col gap-y-2 w-1/3 p-4'>
            <div className='flex'>
                <p className='px-2 py-1 border rounded-xl text-xs bg-lightGray text-steel'>
                {/* 5 min read */}
                {article.views}
                </p>
            </div>
            ...
            <div className='mt-4 flex items-start'>
                <div className="group flex items-center cursor-pointer" style={{ height: '1rem', width: 'auto' }}>
                    <Link href={`/blog/${article._id}`} className="relative text-sm overflow-hidden">
